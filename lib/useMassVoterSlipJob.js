...
import {
  normalizePdfErrorMessage as normalizePdfErrorMessageShared,
  sanitizeMassSlipPayload,
...
function normalizePdfErrorMessage(message = "") {
  return normalizePdfErrorMessageShared(message);
}
...
export default function useMassVoterSlipJob({
  ...
  const startJob = useCallback(
    async (filtersInput) => {
      ...
      const sanitizedBase = sanitizeMassSlipPayload(basePayload);
      if (sanitizedBase.changed) {
        onWarning?.(
