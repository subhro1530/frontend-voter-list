...
function normalizeRecoveryTime(engine) {
  const directMs = toNumber(
    engine?.recoveryTimeMs ??
      engine?.retryAfterMs ??
      engine?.cooldownRemainingMs,
  );
  if (directMs > 0) {
    return {
      recoveryTimeMs: directMs,
      recoveryAt: null,
    };
  }

  const recoveryAtRaw =
    engine?.recoveryAt || engine?.cooldownUntil || engine?.nextAvailableAt;
  const recoveryAt = recoveryAtRaw ? new Date(recoveryAtRaw) : null;
  if (recoveryAt && !Number.isNaN(recoveryAt.getTime())) {
    return {
      recoveryTimeMs: Math.max(0, recoveryAt.getTime() - Date.now()),
      recoveryAt: recoveryAt.toISOString(),
    };
  }
...
export function normalizeApiKeysStatus(payload) {
  ...
  snapshot.allExhausted =
    snapshot.availableEngines <= 0 &&
    snapshot.activeEngines <= 0 &&
    snapshot.rateLimitedEngines <= 0;
 ...
 * @returns {boolean}
 */
export function extractPaidFallbackActive(payload) {
  if (!payload || typeof payload !== "object") return false;
  if (typeof payload.paidFallbackActive === "boolean") {
    return payload.paidFallbackActive;
  }
  const tier = extractDispatchTier(payload);
  return tier === "paid";
}

/**
 * @param {DispatchTier | undefined} tier
 */
export function formatDispatchTierLabel(tier) {
  return tier === "paid" ? "PAID (Fallback Active)" : "FREE (Primary)";
}
