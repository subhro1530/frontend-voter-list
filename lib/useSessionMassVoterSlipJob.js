...
function normalizeJobStatus(status) {
  ...
  if (["queued", "processing", "completed", "failed", "idle"].includes(value)) {
    return value;
  }
  return "idle";
}
...
function mapStartError(error) {
  ...
  if (status >= 500)
    return "Server error while starting generation. Please retry.";
...
function mapDownloadError(error) {
  ...
  if (status >= 500) {
    return "Server error while downloading PDF. Please retry.";
  }
...
export default function useSessionMassVoterSlipJob({
  ...
  const pollJob = useCallback(
    async (jobIdInput) => {
      ...
      try {
        ...
        onWarning?.(error?.message || "Could not refresh job status.");
      }
    },
    [
      applyJobState,
  ...
  const startSessionMassSlip = useCallback(async () => {
    ...
    try {
      const data =
        await userAPI.startMassVoterSlipForSession(resolvedSessionId);
      const job = extractJob(data);
      const responseSession = extractResponseSession(data);
