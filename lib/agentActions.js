...
export class SmartAgent {
  ...
  async processMessage(message) {
    ...
    // Handle expecting input state (user was asked to provide a name/id)
    if (this.context.expectingInput) {
      const inputType = this.context.expectingInput;
      this.context.expectingInput = null;

      if (inputType === "voter_name") {
        return this.handleSearchVoter(message.trim(), message);
      } else if (
        inputType === "voter_id" ||
        inputType === "voter_id_for_print"
      ) {
        return this.handleSearchVoterId(message.trim().toUpperCase(), message);
      }
    }
    ...
    // Store expecting input if needed
    const result = await this.executeIntent(intent, message);
    if (result.expectingInput) {
      this.context.expectingInput = result.expectingInput;
    }
  ...
  async detectIntent(message) {
    ...
    // Generic "search for a voter" - no specific name, ask for it
    if (/^search\s*(for\s*)?(a\s+)?voter$/i.test(lowerMessage)) {
      return { type: "search_voter", name: null };
    }

    // Search by voter ID / EPIC - check first as it's more specific
    if (/voter\s*id|epic|card\s*number|id\s*number/i.test(lowerMessage)) {
      const idMatch = message.match(/[A-Z]{2,3}\d{6,10}/i);
      return { type: "search_voter_id", voterId: idMatch?.[0] || null };
    }

    // Direct voter ID in message
    const voterIdMatch = message.match(/\b([A-Z]{2,3}\d{6,10})\b/i);
    if (voterIdMatch) {
      return { type: "search_voter_id", voterId: voterIdMatch[1] };
    }

    // Search voter by name - more flexible patterns
    if (
      /search|find|look\s*for|show\s*me|get|fetch|is\s*there/i.test(
        lowerMessage,
      )
    ) {
      ...
      // "search for ramesh" or "find ramesh" or "search ramesh"
      const namePatterns = [
        /(?:search|find|look\s*for|get|fetch)\s+(?:for\s+)?(?:voter\s+)?(?:named?\s+)?["']?([a-z\s]+?)["']?$/i,
        /(?:is\s+there\s+(?:a\s+)?(?:voter\s+)?(?:named?\s+)?)["']?([a-z\s]+?)["']?\??$/i,
        /(?:search|find)\s+(?:for\s+)?["']?([a-z][a-z\s]+)["']?$/i,
      ];
      ...
      for (const pattern of namePatterns) {
        ...
        if (match && match[1]) {
          ...
          if (!/^(a\s+)?voter$|^person$|^someone$|^anyone$/i.test(name)) {
            break;
          }
          name = null;
        }
...
💡 **Tip**: You can ask me in natural language - I understand context!`,
  ...
  async handleAIQuery(message) {
    // If the message looks like just a name (no special keywords), try searching
    const cleanMessage = message.trim();
    if (
      /^[a-zA-Z\s]{2,30}$/.test(cleanMessage) &&
      !/^(hi|hello|hey|yes|no|ok|help)$/i.test(cleanMessage)
    ) {
      // Looks like a name - try searching
      return this.handleSearchVoter(cleanMessage, message);
    }
