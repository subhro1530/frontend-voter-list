...
export const electionResultsAPI = {
  ...
  getSessions: (signal) =>
    fetchWithRetry("/election-results/sessions", { method: "GET", signal }),
  ...
  exportExcel: async (id) => {
    const token = getAuthToken();
    const url = `${API_PREFIX}/election-results/sessions/${id}/export/excel`;
    const res = await fetch(url, {
...
export const affidavitAPI = {
  ...
  exportDocx: async (id, candidateName) => {
    const token = getAuthToken();
    const url = `${API_PREFIX}/affidavits/sessions/${id}/export/docx`;
    const res = await fetch(url, {
      headers: token ? { Authorization: `Bearer ${token}` } : {},
    });
    if (!res.ok) {
